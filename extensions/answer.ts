...
async function selectExtractionModel(
  ...
  const fallbackModel = modelRegistry.find("zai", FALLBACK_MODEL_ID);
  if (!fallbackModel) {
    return currentModel;
  }

  const auth = await modelRegistry.getApiKeyAndHeaders(fallbackModel);
  if (auth.ok === false) {
    return currentModel;
  }
...
export default function (pi: ExtensionAPI) {
  const answerHandler = async (ctx: ExtensionContext) => {
    ...
    // Select the best model for extraction (prefer GPT-5.4-mini, then glm-5.1)
